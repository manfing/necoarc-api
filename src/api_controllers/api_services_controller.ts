...
export abstract class ApiServicesController {
    ...
    public static Create_session(_session_name: string/* , _choosenApi: CurrentApi */): boolean {
        try {
            ...
            console.log(`⚡️[Neco]: Não foi possível criar sessão ${_session_name}: ${error}`);
            return false;
        }
    }
    ...
    public static Remove_session(_session_name: string, _session?: CurrentApi): boolean {
        let running_session;
